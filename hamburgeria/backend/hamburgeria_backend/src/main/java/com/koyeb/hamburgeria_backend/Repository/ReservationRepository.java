...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("SELECT TO_CHAR(r.bookedDate, 'YYYY-MM') AS month, COUNT(r.id) AS total FROM Reservation r WHERE EXTRACT(YEAR FROM r.bookedDate) = :year GROUP BY TO_CHAR(r.bookedDate, 'YYYY-MM') ORDER BY month")
    List<Object[]> findMonthlyReservationCountByYear(int year);

    @Query("SELECT TO_CHAR(r.bookedDate, 'YYYY-MM-DD') AS day, COUNT(r.id) AS total FROM Reservation r WHERE EXTRACT(YEAR FROM r.bookedDate) = :year AND EXTRACT(MONTH FROM r.bookedDate) = :month GROUP BY TO_CHAR(r.bookedDate, 'YYYY-MM-DD') ORDER BY day")
    List<Object[]> findDailyReservationCountByYearAndMonth(int year, int month);

    @Query("SELECT TO_CHAR(r.bookedDate, 'YYYY-MM-DD HH24') AS hour, COUNT(r.id) AS total FROM Reservation r WHERE EXTRACT(YEAR FROM r.bookedDate) = :year AND EXTRACT(MONTH FROM r.bookedDate) = :month AND EXTRACT(DAY FROM r.bookedDate) = :day GROUP BY TO_CHAR(r.bookedDate, 'YYYY-MM-DD HH24') ORDER BY hour")
